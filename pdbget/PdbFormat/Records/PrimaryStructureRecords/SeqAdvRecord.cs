...
public class SeqAdvRecord : AbstractChainedRecord
{
    ...
    public override void FromRow(string row)
    {
        ...
        ChainId = row[16];                                       // 17
        if (int.TryParse(row.AsSpan(18, 4), out int seqNum))  // 19 - 22
            SeqNum = seqNum;
        ICode = row[22];                                         // 23
        Database = row.Substring(24, 4).Trim();                  // 25 - 28
        DbAccession = row.Substring(29, 9).Trim();               // 30 - 38
        DbRes = row.Substring(39, 3).Trim();                     // 40 - 42
        if (int.TryParse(row.AsSpan(43, 5), out int dbSeq))
            DbSeq = dbSeq;                                       // 44 - 48
        SeeRemark999 = row[49..].Trim() == "CONFLICT SEE REMARK 999";
        if (!SeeRemark999)
